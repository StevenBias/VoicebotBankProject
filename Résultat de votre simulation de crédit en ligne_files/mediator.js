...
Mediator = new function () {
    ...
    this.init = function () {
        ...
        if (typeof (sfIaParam) != "undefined" && typeof (sfIaParam.version) == "undefined") {
    ...
    this.sendPostData = function (topic, data, url) {
		...
		if(data.Json) { console.log(data.Json); }
        var form = document.createElement("form");
        form.setAttribute("method", "POST");
        form.setAttribute("action", url);
        if(typeof (data.openin) != "undefined"){
    		form.setAttribute("target", data.openin);
        }
    ...
    this.sendData = function (topic, data, url) {
        ...
        if (iaId && (typeof registeredIa[iaId] != "undefined")) {
			...
			if(data.ModuleName && (data.ModuleName === "newEpartnerResult" || data.ModuleName === "newEpartnerSlider"))
				url = url + "?codevendeur="+ data.Json.URL_numberDuVendeur;
			if(url ==="/fr/partner/partnare" && data.ModuleName && (data.ModuleName === "resultBlock"))
				url = url + "?codevendeur="+ data.Json.URL_numberDuVendeur+"&codemateriel="+data.Json.URL_materialCode;
			if(url ==="/fr/credit/souscription-credit" && (data.ModuleName === "simulationResultBlock")){
				url = url + "?ciid="+ data.Json.gtmBtnName+"|"+data.Json.gtmPgName;
			}
			if(data && data.Json && data.Json.vosFract === undefined) data.Json.vosFract ="-";
	...
	this.getregisteredIa=function(topic){
        var iaId = this.getIaIdFromTopic(topic);
        return registeredIa[iaId];
    }
